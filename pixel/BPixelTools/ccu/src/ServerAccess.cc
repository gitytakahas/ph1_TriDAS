  ...
  try {
	...
	if (ccuList != NULL) {
	  for (std::list<keyType>::iterator p=ccuList->begin() ; p!=ccuList->end() ; p++) {
	    ...
	    // cout <<"CCU_" <<ccuAddress<< " SRA " <<SRA << endl; 
	    // cout <<"CCU_" <<ccuAddress<< " SRB " <<SRB << endl;
	    // cout <<"CCU_" <<ccuAddress<< " SRC " <<SRC << endl;
	    // cout <<"CCU_" <<ccuAddress<< " SRD " <<SRD << endl; 
	    // cout <<"CCU_" <<ccuAddress<< " SRE " <<SRE << endl;
	    // cout <<"CCU_" <<ccuAddress<< " SRF " <<SRF << endl;
	    // cout <<"CCU_" <<ccuAddress<< " CRA " <<CRA << endl;
	    // cout <<"CCU_" <<ccuAddress<< " CRB " <<CRB << endl; 
	    // cout <<"CCU_" <<ccuAddress<< " CRC " <<CRC << endl;
	    // cout <<"CCU_" <<ccuAddress<< " CRD " <<CRD << endl;
	    // cout <<"CCU_" <<ccuAddress<< " CRE " <<CRE << endl;
...
string DCDCdisable      (FecAccess   *fecAccess , 
 ...
 if((piaInMask[ piaIndex ] ^ piaValue) != piaPGPattern[ piaIndex ] ) {
   //if( piaValue != piaPGPattern[ piaIndex ] ) {
          if( !printed ) {
	    cout << (piaInMask[ piaIndex ] ^ piaValue) << " "<<  piaPGPattern[ piaIndex ] << endl;
            o << " Enable Selection = " << bitPattern << std::endl;
            printed = true;
          }
...
//##############################################################################
//##############################################################################
string ReadQpll (  FecAccess   *fecAccess  , 
			 tscType8     fecAddress ,
			 tscType8     ringAddress, 
			 tscType8     ccuAddress,
			 unsigned int dcdcAddress,
  ...
  keyType ccuChannelKey;
  tscType32 ccuCRE;
  tscType16 ddr, piaData, piaIndex=1;
  tscType16 piaChannel = 0x30;
  tscType16 piaDDR  =  0x06;
  ...
  try {
      ...
      if( ccuCRE != 0xF0000 ) {
        ...
        ccuChannelKey = ccuAddressKey | setChannelKey( piaChannel );
	fecAccess->setPiaChannelDDR( ccuChannelKey, piaDDR );
        ddr = fecAccess->getPiaChannelDDR( ccuChannelKey );
        if( ddr != piaDDR ) {
          printf( "CCU 0x%02X DDR%c=0x%02X (0x%02X)\n", ccuAddress, 'A' + piaIndex, ddr, piaDDR );
          return o.str();}

        piaData = fecAccess->getPiaChannelDataReg( ccuChannelKey ) & piaDDR;
    ...
    std::cerr << textBuffer;
    exit( EXIT_FAILURE );
    cout << "FecExceptionHandler"<< endl;
  }

  tscType16 piaValue;
  tscType16 piaDDR1;
  tscType16 piaInMask  = { 0x18 };
  tscType16 piaPGPattern[2];
  tscType16 i;
  char bitPattern[2]={0};
 
  struct { tscType16 pia;
           tscType16 bit; } pg[ 2 ] = { { 0, 0x08 },   //PGC1_0 08 0, 0x40
                                        { 0, 0x10 } }; //PGC2_6 10 0, 0x80
...
for( i = 0; i < 2; i++ ){
    piaDDR1 = fecAccess->getPiaChannelDataReg( ccuChannelKey ) & piaDDR;
    piaValue = fecAccess->getPiaChannelDataReg( ccuChannelKey ) & piaInMask;
    }
   
if(piaValue ==piaInMask){
  cout << "ok" << endl;
  printf( "qpll locked");}
else{  
  printf("qpll NOT locked \n piaInMask %d \n piaValue %d",piaInMask, piaValue);} 
//delete fecAccess;
 return o.str();
...
string DCDCenableTest (  FecAccess   *fecAccess  , 
  ...
  tscType16 piaChannel[ 4 ] = { 0x30, 0x31, 0x32, 0x33 };
  tscType16 piaDDR[ 4 ]     = { 0x06, 0x39, 0x8E, 0x67 };
  // tscType16 piaDDR[ 4 ]     = { 0x03, 0x39, 0x8E, 0x67 };
  try {
  ...
  try {
	...
	if( enPattern & mask ) {
      ...
      for( piaIndex = 0; piaIndex < 4; piaIndex++ ) {
        ccuChannelKey = ccuAddressKey | setChannelKey( piaChannel[ piaIndex ] );
	 piaValue = fecAccess->getPiaChannelDataReg( ccuChannelKey ) & piaInMask[ piaIndex ];
	 // cout << "ccuChannelKey  " <<ccuChannelKey << endl;
	
	 // cout <<  " piavalue fecAccess->getPiaChannelDataReg( " <<ccuChannelKey <<") & piaInMask[ piaIndex ]"  <<
	 //fecAccess->getPiaChannelDataReg( ccuChannelKey ) << "  " <<  piaInMask[ piaIndex ] << endl;
	 // cout << " piavalue " << piaValue <<  " PG pattern "<<piaPGPattern[ piaIndex ] << endl;
	 // cout << "hope " << (piaInMask[ piaIndex ] ^ piaValue) << " "<< piaPGPattern[ piaIndex ] << endl;
        if((piaInMask[ piaIndex ] ^ piaValue) != piaPGPattern[ piaIndex ] ) {
	  //if( piaValue != piaPGPattern[ piaIndex ] ) {
          if( !printed ) {
            o << " Enable Selection = " << bitPattern << std::endl;
