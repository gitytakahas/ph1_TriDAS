...
const char* SysCommand::GetWord(const char *s, int* l)
{
	...
	//char temp[256]={0}; strncpy(temp,word,*l); temp[*l]='\0'; cout << "the word is [" << temp << "]" << endl;
	return word;
}
...
//------------------------------------------------------------
bool SysCommand::ParseNumberOrAlias(const char* word, const int &l, int *id){
...
int SysCommand::Parse(const char * line)
{
	...
	{
	  if (ParseNumberOrAlias(word,l,&i1)){
	      ...
	      if( (l>0) && (strncmp(word,":",l)==0) )
		{
		  ...
		  if ( !ParseNumberOrAlias(word,l,&i2) ){
	    ...
	    } while ( (l>0) && (kbuf<999) && ParseNumberOrAlias(word,l,&i1) );
	...
	*/
	...
	// not a Parser command, look at system commands:
	// the structure is
	// [target][command]
	// where target=[tb|sys|{[cn cn-id][module module-id][roc rod-id|tbm tbm-id]}
	int consumed=0;
	target.type=kNONE;

	if( (narg0>consumed) && (strcmp(carg0[consumed],"tb")==0) ){
	...
	}else{// target of the type [cn] [module] [roc,tbm] ...
 
	  // optional control network identifier "cn"
	  if(strcmp(carg0[0],"cn")==0){
	    target.type=kCN; // unless overridden by subsequent targets
	    target.nModule=0; 
	    target.nRoc=0;
	    consumed++;
	    ...
	    if( (narg0>consumed) && (iarg0[consumed]!=NULL) ){
	      ...
	      target.nCN=0;
	      for(int* j=iarg0[consumed]; ((*j)>=0)&&(target.nCN<nCNMax); j++){
		target.CN[target.nCN++]=*j;
	      }
	      consumed++;
	    }else {
	      // no id given, means we want to talk to the current default cn
	      target.nCN=defaultTarget.nCN;
	      for(int i=0; i<target.nCN; i++) target.CN[i]=defaultTarget.CN[i];
	    }
	  } else{ // control network keyword absent, use default
	    target.nCN=defaultTarget.nCN;
	    for(int i=0; i<target.nCN; i++) target.CN[i]=defaultTarget.CN[i];
	  }
	  ...
	  // Next is the optional module identifier
 	  if((narg0>consumed) && strcmp(carg0[consumed],"module")==0){
	    target.type=kMOD; // this doesn't really exits
	    target.nRoc=0;
	    consumed++;
	    // always expand target lists
	    if( (narg0>consumed) && (iarg0[consumed]!=NULL) ){
	  ...
	  }else{ // module keyword absent, use default  unless a cn has been given
	    if(target.type==kNONE){
	      target.nModule=defaultTarget.nModule;
	      for(int i=0; i<target.nModule; i++) target.hub[i]=defaultTarget.hub[i];
	    }else{
	      target.nModule=0;
	  ...
	  // optional target identifier tbm or roc
	  if( (narg0>consumed) && (strcmp(carg0[consumed],"tbm")==0) ) {
	    if(target.type==kCN){ cout << "warning: module id missing" << endl;  }
	    target.type=kTBM;
	    target.nRoc=0;
	    consumed++;
	...
	if(consumed==narg0){
	  ...
	  //cout << "target updated  cn=" << target.CN[0] << endl;
	  narg=0;
	  ...
	  type=kNONE; 
	  return 1;  // return to update the prompt
	  ...
	  for(int j=0; j<narg; j++){
	    ...
	    if(iterate[j+consumed]>0){
	      if(iterate[j+consumed]==target.nRoc*target.nModule*target.nCN){
...
// 	    && ( (iHub<target.nModule-1)||(iRoc<target.nRoc-1)) )
  if( ( narg>0 ) && (iCN<target.nCN)
      && ( (iCN<target.nCN-1)|| (iHub<target.nModule-1)||(iRoc<target.nRoc-1)) )
    {

      // increment cn/module/roc id
      iRoc++;
      if(iRoc>=target.nRoc){
	iRoc=0; iHub++;
	if(iHub>=target.nModule){
	  iHub=0; iCN++;
	}
      }
    ...
    }else{ // no more targets from previously parsed line
...
int SysCommand::PrintTarget(char* buf, target_t* t, int mode){
  ...
  if(mode&0x2 && t->type!=kSYS){
    // prepend control network
    l+=sprintf(&buf[l],"{");
    if(t->nCN<5){
      l+=sprintf(&buf[l],getCNname(t->CN[0]).c_str());
      for(int i=1; i<t->nCN; i++){
	l+=sprintf(&buf[l],",");
	l+=sprintf(&buf[l],getCNname(t->CN[i]).c_str());
      }
    }else{
  ...
  if(t->type==kTB){
    ...
    l+=sprintf(&buf[l],"%s","cn");
  }else if((t->type==kTBM) ||(t->type==kROC)||(t->type==kMOD)) {
    if(mode&0x01){
		// add module id 
      l+=sprintf(&buf[l],"[");
...
int SysCommand::getCNidx(const std::string& s){
...
std::string SysCommand::getCNname(const int idx){
  std::ostringstream o;
  int phi=(idx%8)+1;
...
bool SysCommand::Keyword(const char* keyword, int** value1, int** value2, int** value3){
  if(   (carg[0]!=NULL) && (strcmp(carg[0],keyword)==0)
	&& (narg==4)
	&& (iarg[1]!=NULL) && (*(iarg[1])>-1)
	&& (iarg[2]!=NULL) && (*(iarg[2])>-1)
	&& (iarg[3]!=NULL) && (*(iarg[3])>-1) )
  {
    *value1=iarg[1];
    *value2=iarg[2];
    *value3=iarg[3];
...
bool SysCommand::Keyword(const char* keyword, int** value1, int** value2, const char* keyword1){
  if(   (carg[0]!=NULL) && (strcmp(carg[0],keyword)==0)
	&& (narg==4)
	&& (iarg[1]!=NULL) && (*(iarg[1])>-1)
	&& (iarg[2]!=NULL) && (*(iarg[2])>-1)
	&& (carg[3]!=NULL) && (strcmp(carg[3],keyword1)==0) )
  {
    *value1=iarg[1];
    *value2=iarg[2];
    return true;
...
bool SysCommand::Keyword(const char* keyword, const char* keyword1, int** value){
  if(   (carg[0]!=NULL) && (strcmp(carg[0],keyword)==0)
	&& (narg==3)
	&& (carg[1]!=NULL) && (strcmp(carg[1],keyword1)==0)
	&& (iarg[2]!=NULL) && (*(iarg[2])>-1) )  
