...
def showRMSPOH(sector,poh,gain,slopethr,wait=True,filename=None):
  ...
  for ch in range(1,5):
     ...
     alias="POH "+ str(poh)+":"+str(ch)+" fed channel "+str(sector.poh[poh].fedchannels[ch])
     biasGraph=graph([(t[0],t[1]) for t in data[ch]],    style=20, color=1, name=alias)
...
#################################################
  return sector.poh[poh].biasGraphs
  #c1.Print(filename)
  gCanvases.append(c1)
