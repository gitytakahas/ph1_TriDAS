...
/*
 * $Log: SAXCount.cpp,v $
 * Revision 1.2  2007/11/07 14:13:19  fdrouhin
 * Newest version of the FecSoftware, XML/DB accessed have been changed drasticly, see Version.txt and README.TXT
 *
 * Revision 1.1  2006/12/12 16:07:12  fdrouhin
	...
	if (argInd < argC)
	  ...
	  if (!strcmp(xmlFile,"database")) {
		...
		if (resultLength) {
		  ...
		  std::cout << "Buffer allocation took " <<  (endMillis-startMillis) << " ms" << std::endl ;
		  duration += endMillis-startMillis ;
		  ...
		  std::cout << "Buffer creation took " << (endMillis-startMillis) << " ms" << std::endl ;
		  duration += endMillis-startMillis ;

		  startMillis = XMLPlatformUtils::getCurrentMillis();
		  parser->parse(*((InputSource*)xmlInputSource)) ;
		  endMillis = XMLPlatformUtils::getCurrentMillis();
	  ...
	  std::cout << "Found " << handler.getCountPIA() << " PIA" << std::endl ;
	  std::cout << "Found " << handler.getCountAPV25() << " APV25" << std::endl; 
	  std::cout << "Found " << handler.getCountAPVMUX() << " APVMUX" << std::endl ;
	  std::cout << "Found " << handler.getCountDCUCCU() << " DCU on CCU" << std::endl ;
	  std::cout << "Found " << handler.getCountDCUFEH() << " DCU on FEH" << std::endl ;
	  std::cout << "Found " << handler.getCountAOH() << " AOH" << std::endl ;
	  std::cout << "Found " << handler.getCountDOH() << " DOH" << std::endl ;
	  std::cout << "Found " << handler.getCountPLL() << " PLL" << std::endl ;
        }
      else
	errorOccurred = true;
    }
