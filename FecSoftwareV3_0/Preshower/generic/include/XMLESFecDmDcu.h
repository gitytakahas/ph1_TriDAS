...
#ifndef XMLESFECDMDCU_H
#define XMLESFECDMDCU_H
 ...
 */
class XMLESFecDmDcu : public XMLESFecDevice {

 public:
  /** \brief Default constructor
   */
  XMLESFecDmDcu ()  throw (FecExceptionHandler);

#ifdef DATABASE
  /** \brief Retreive an XML input source from database
   */
  XMLESFecDmDcu ( ESDbFecAccess *dbAccess )  throw (FecExceptionHandler) ;
#endif

  /** \brief Constructor with xml buffer
   */
  XMLESFecDmDcu (const XMLByte* xmlBuffer ) throw (FecExceptionHandler) ;

  /** \brief Constructor with file access
   */
  XMLESFecDmDcu ( std::string xmlFileName ) throw (FecExceptionHandler) ;
