...
#ifndef XMLESFECDCU_H
#define XMLESFECDCU_H
 ...
 */
class XMLESFecDcu : public XMLESFecDevice {

 public:
  /** \brief Default constructor
   */
  XMLESFecDcu ()  throw (FecExceptionHandler);

#ifdef DATABASE
  /** \brief Retreive an XML input source from database
   */
  XMLESFecDcu ( ESDbFecAccess *dbAccess )  throw (FecExceptionHandler) ;
#endif

  /** \brief Constructor with xml buffer
   */
  XMLESFecDcu (const XMLByte* xmlBuffer ) throw (FecExceptionHandler) ;

  /** \brief Constructor with file access
   */
  XMLESFecDcu ( std::string xmlFileName ) throw (FecExceptionHandler) ;
