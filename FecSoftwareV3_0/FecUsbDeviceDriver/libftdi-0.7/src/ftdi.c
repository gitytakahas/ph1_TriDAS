...
int ftdi_usb_open_dev(struct ftdi_context *ftdi, struct usb_device *dev)
{
	...
	break;
      ...
      if ( jj == 299 && !(ftdi->usb_dev = usb_open(dev) ) ) {
	printf( " tried 299 times  but still failed to open usb \n");
	ftdi_error_return(-4, "usb_open() failed");
      }
    }
    if ( jj!= 0 ) 
      printf( " *****  tried %d times before opening usb device \n", jj);

/*     for(jj=0; jj<(ftdi->usb_claim_timeout/10); jj++) { */
/*       if (usb_claim_interface(ftdi->usb_dev, ftdi->interface) != 0) { */
/* 	usleep(10000); */
/*       }else {   */
/* 	break; */
/*       } */
/*       if ( jj == 299 && usb_claim_interface(ftdi->usb_dev, ftdi->interface) != 0) { */
/* 	// printf( " tried 299 times  but still failed to claim usb \n"); */
/*         usb_release_interface(ftdi->usb_dev, ftdi->interface); // RSLU added 24.10.2007 */
/* 	usb_close (ftdi->usb_dev); */
/*         ftdi_error_return(-5, "unable to claim usb device. Make sure ftdi_sio is unloaded!"); */
/*       } */
/*     } */
/*     if ( jj!= 0 )  */
/*       printf( " *****  tried %d times before claiming usb device \n", jj); */
/*     // -----   RSLU */
...
/*     usb_release_interface(ftdi->usb_dev, ftdi->interface); // RSLU added 24.10.2007 */
/*     //usb_close(ftdi->usb_dev); // RSLU added 24.10.2007 */
                       ...
                       const char* description, const char* serial)
{
    ...
    for (bus = usb_busses; bus; bus = bus->next) {
		...
		if ( jj!= 0 ) 
		  printf( " *****  tried %d times to open usb device \n", jj);
...
int ftdi_usb_claim(struct ftdi_context *ftdi){
  int status;
  ...
  do {
    ...
    if (status = usb_claim_interface(ftdi->usb_dev, ftdi->interface)) {
      // give a breath to the cpu...
      sched_yield();
      gettimeofday(&stop_time,NULL);
      timersub(&stop_time,&start_time,&diff_time);
      /*timeout break: */
      if ((diff_time.tv_sec * 1000 + diff_time.tv_usec/1000) 
	  > ftdi->usb_claim_timeout ) {
	printf("usb claim : timeout occurred. (%d msec)\n"
	       ,ftdi->usb_claim_timeout);
	return (-ETIME);
      }
    }
...
int ftdi_usb_release(struct ftdi_context *ftdi){
  return usb_release_interface(ftdi->usb_dev, ftdi->interface);
}
...
int ftdi_get_serial_string (struct ftdi_context *ftdi, char *string, int buflen) {


  struct usb_device *dev = usb_device(ftdi->usb_dev);
  if ( ftdi->usb_dev = usb_open(dev) ) {
    if (dev) {
      if (usb_get_string_simple(ftdi->usb_dev, dev->descriptor.iSerialNumber, string, buflen) <= 0) {
	printf(" in ftdi_get_serial_string, can not get serial number(internal).\n");
	ftdi_error_return(-9, "can not get serial number");
      }
    } else {
      ftdi_error_return(-2, "can not get serial number, no device found");
    }
  }else {
    printf(" in ftdi_get_serial_string, failed to open usb \n");
    ftdi_error_return(-1, "can not open usb device \n");
  }
  return 0;
}
